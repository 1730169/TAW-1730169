<div class="text-nowrap text-md-right">
    <a href="{{ route('torneos.show', $torneo->id) }}" class="btn {{ !request()->ajax() ? 'btn-primary' : 'btn-link text-secondary p-1' }}" title="{{ __('Show') }}">
        <i class="far fa-eye {{ !request()->ajax() ? 'fa-fw' : '' }}"></i>
    </a>

    <a href="{{ route('torneos.edit', $torneo->id) }}" class="btn {{ !request()->ajax() ? 'btn-primary' : 'btn-link text-secondary p-1' }}" title="{{ __('Edit') }}">
    ...
    <form method="post" action="{{ route('torneos.destroy', $torneo->id) }}" id="delete_torneo_{{ $torneo->id }}_form" class="d-none">
        @csrf
        @method('delete')
    </form>
</div>
